...
let TaskService = class TaskService {
    ...
    async checkAuth() {
        try {
            const ipResponse = await axios_1.default.get('https://www.ipplus360.com/getIP');
            const serverIp = ipResponse.data.data;
            if (!serverIp) {
                common_1.Logger.error('Failed to retrieve server IP.', 'TaskService');
                this.isAuthorized = false;
                return;
            }
            const authResponse = await axios_1.default.get(`https://auth.chuanchuan.cloud/verify?ip=${serverIp}`);
            const { valid, code } = authResponse.data;
            const internalAuthCode = process.env.INTERNAL_AUTH_CODE;
            this.isAuthorized = valid && code === internalAuthCode;
            if (this.isAuthorized) {
                common_1.Logger.log('授权成功', 'TaskService');
            }
            else {
                common_1.Logger.error('授权码错误，你可能真正该使用盗版系统程序系统！如需授权或者购买源码，请联系作者微信：hxgsrubxjogxeeag', 'TaskService');
            }
        }
        catch (error) {
            common_1.Logger.error(`Error in authorization check: ${error.message}`, 'TaskService');
            this.isAuthorized = false;
        }
    }
    getAuthStatus() {
...
__decorate([
    (0, schedule_1.Cron)('0 0 0 */30 * *'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], TaskService.prototype, "checkAuth", null);
