...
let MidjourneyService = class MidjourneyService {
    ...
    async updateDrawData(jobData, drawRes) {
        try {
            ...
            throw new common_1.HttpException('此次绘制失败，原因：存储图片失败，请重试！', common_1.HttpStatus.BAD_REQUEST);
        }
    }
    ...
    async pollComparisonResultDraw(drawInfo) {
        ...
        try {
            ...
            throw new common_1.HttpException('此次绘制失败，原因：获取图片列表结果失败，请重试！', common_1.HttpStatus.BAD_REQUEST);
        }
    }
    ...
    async pollComparisonResultReGenerate(drawInfo) {
        ...
        let count = 0;
        let isMatchSuccessful = false;
        while (!isMatchSuccessful && pollingTime < TIMEOUT) {
            ...
            isMatchSuccessful = reGenerateImgDetail && !reGenerateImgDetail.edited_timestamp;
    ...
    async findCurrentReGenerateImgResult(randomDrawId, message_id) {
        ...
        const reGenerateImgDetail = messageList.find((item) => {
            const { attachments = [], content, edited_timestampd } = item;
            return content.includes(randomDrawId) && attachments.length > 0 && !histroyMessageIds.includes(item.id) && item.id !== message_id && this.isReGenerateImage(content);
        });
    ...
    async getDrawActionDetail(action, drawId, orderId) {
        const findCustomID = (components, custom_ID) => {
            for (const component of components) {
                if (component.type === 1 && component.components) {
                    const found = findCustomID(component.components, custom_ID);
                    if (found)
                        return found;
                }
                else if (component.type === 2 && component.custom_id.includes(custom_ID)) {
                    return component;
                }
            }
            return null;
        ...
        if (action === midjourney_constant_1.MidjourneyActionEnum.VARY) {
            if (orderId === 3) {
                currentImgComponent = findCustomID(components, 'low_variation');
            }
            if (orderId === 4) {
                currentImgComponent = findCustomID(components, 'high_variation');
            }
        }
        if (action === midjourney_constant_1.MidjourneyActionEnum.ZOOM) {
            if (orderId === 1) {
                currentImgComponent = findCustomID(components, 'Outpaint::50');
            }
            if (orderId === 2) {
                currentImgComponent = findCustomID(components, 'Outpaint::75');
            }
        }
